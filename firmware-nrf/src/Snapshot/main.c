...
static int pn532_wakeup(void)
{
        ...
        return 0; // Always return success, wakeup is best-effort
}
...
static int pn532_read_ack(void)
{
        ...
        // ACK is optional in I2C mode, just return success
...
static int pn532_read_response(uint8_t *resp, uint8_t *resp_len, uint16_t timeout_ms)
{
        ...
        if (ret != 0)
        {
                ...
                // Continue anyway, PN532 might send response directly
        }
            ...
            frame[2] != PN532_STARTCODE2)
        {
                ...
                LOG_HEXDUMP_ERR(frame, 10, "Frame:");
                return -EINVAL;
        }

        uint8_t len = frame[3];
        *resp_len = len - 2; // Subtract TFI and checksum
        memcpy(resp, &frame[6], *resp_len);
